import React, { useEffect } from 'react';
import { connect } from 'react-redux';
import { Link, RouteComponentProps } from 'react-router-dom';
import { Button, Row, Col } from 'reactstrap';
import {} from 'react-jhipster';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';

import { IRootState } from 'app/shared/reducers';
import { getEntity } from './objective.reducer';
import { APP_DATE_FORMAT, APP_LOCAL_DATE_FORMAT } from 'app/config/constants';

export interface IObjectiveDetailProps extends StateProps, DispatchProps, RouteComponentProps<{ id: string }> {}

export const ObjectiveDetail = (props: IObjectiveDetailProps) => {
  useEffect(() => {
    props.getEntity(props.match.params.id);
  }, []);

  const { objectiveEntity } = props;
  return (
    <Row>
      <Col md="8">
        <h2 data-cy="objectiveDetailsHeading">Objective</h2>
        <dl className="jh-entity-details">
          <dt>
            <span id="id">ID</span>
          </dt>
          <dd>{objectiveEntity.id}</dd>
          <dt>
            <span id="name">Name</span>
          </dt>
          <dd>{objectiveEntity.name}</dd>
          <dt>
            <span id="note">Note</span>
          </dt>
          <dd>{objectiveEntity.note}</dd>
          <dt>
            <span id="userLogin">User Login</span>
          </dt>
          <dd>{objectiveEntity.userLogin}</dd>
          <dt>
            <span id="amountTot">Amount Tot</span>
          </dt>
          <dd>{objectiveEntity.amountTot}</dd>
          <dt>
            <span id="amountVar">Amount Var</span>
          </dt>
          <dd>{objectiveEntity.amountVar}</dd>
        </dl>
        <Button tag={Link} to="/objective" replace color="info" data-cy="entityDetailsBackButton">
          <FontAwesomeIcon icon="arrow-left" /> <span className="d-none d-md-inline">Back</span>
        </Button>
        &nbsp;
        <Button tag={Link} to={`/objective/${objectiveEntity.id}/edit`} replace color="primary">
          <FontAwesomeIcon icon="pencil-alt" /> <span className="d-none d-md-inline">Edit</span>
        </Button>
      </Col>
    </Row>
  );
};

const mapStateToProps = ({ objective }: IRootState) => ({
  objectiveEntity: objective.entity,
});

const mapDispatchToProps = { getEntity };

type StateProps = ReturnType<typeof mapStateToProps>;
type DispatchProps = typeof mapDispatchToProps;

export default connect(mapStateToProps, mapDispatchToProps)(ObjectiveDetail);
